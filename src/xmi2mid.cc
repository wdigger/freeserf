...
public:
  ...
  bool operator()(midi_node_t*& m1, midi_node_t*& m2) {
    if (m1->time != m2->time) {
...
typedef struct {
  std::priority_queue<midi_node_t*, std::vector<midi_node_t*>, compare_by_time> nodes;
  uint32_t tempo;
  uint8_t *data;
  uint64_t size;
} midi_file_t;
...
xmi_process_single(char *data, int length, midi_file_t *midi)
{
  ...
  LOGV("xmi2mid", "Processing XMI chunk: %.4s", (char *)&string);
  ...
  if (0 == processed) {
    LOGD("xmi2mid", "Unknown XMI chunk: %s (0x%X)", string, name);
  }
  return size+4;
}
...
xmi2mid(void *xmi, size_t xmi_size, size_t *mid_size)
{
  ...
  xmi_process_subchunks((char*)xmi, (int)xmi_size, &midi_file);
  void *mid = (char*)midi_produce(&midi_file, mid_size);
