...
{
	uint8_t *data = (uint8_t*)malloc(210);
	...
	game.map.size = *(uint16_t *)&data[190];
	...
	game.map.col_size = 5 + game.map.size/2;
	game.map.row_size = 5 + (game.map.size - 1)/2;
	...
	game.map.update_map_initial_pos = load_v0_map_pos(map, *(uint32_t *)&data[68]);
...
load_v0_player_state(FILE *f)
{
	...
	uint8_t *data = (uint8_t*)malloc(8628);
	if (data == NULL) return -1;
	...
	for (int i = 0; i < 4; i++) {
		...
		game.player[i] = (player_t*)calloc(1, sizeof(player_t));
...
load_v0_map_state(FILE *f, const v0_map_t *map)
{
	...
	uint8_t *data = (uint8_t*)malloc(8*tile_count);
	if (data == NULL) return -1;
...
load_v0_serf_state(FILE *f, const v0_map_t *map)
{
	...
	/* Load serf data. */
	uint8_t *data = (uint8_t*)malloc(16*game.max_serf_index);
	if (data == NULL) return -1;
	...
	for (uint i = 0; i < game.max_serf_index; i++) {
...
load_v0_flag_state(FILE *f)
{
	...
	uint8_t *flag_bitmap = (uint8_t*)malloc(bitmap_size);
	...
	/* Load flag data. */
	uint8_t *data = (uint8_t*)malloc(70*game.max_flag_index);
	if (data == NULL) return -1;
	...
	for (uint i = 0; i < game.max_flag_index; i++) {
...
load_v0_building_state(FILE *f, const v0_map_t *map)
{
	...
	/* Load building data. */
	uint8_t *data = (uint8_t*)malloc(18*game.max_building_index);
	if (data == NULL) return -1;
	...
	for (uint i = 0; i < game.max_building_index; i++) {
...
load_v0_inventory_state(FILE *f)
{
	...
	/* Load inventory data. */
	uint8_t *data = (uint8_t*)malloc(120*game.max_inventory_index);
	if (data == NULL) return -1;
...
{
	fprintf(f, "%s=", name);
	for (int i = 0; i < (int)size-1; i++) fprintf(f, "%i,", values[i]);
...
save_text_game_state(FILE *f)
{
	...
	save_text_write_map_pos(f, "update_map_initial_pos", game.map.update_map_initial_pos);
	...
	save_text_write_value(f, "update_map_last_tick", game.map.update_map_last_tick);
...
save_text_inventory_state(FILE *f)
{
	for (uint i = 0; i < game.max_inventory_index; i++) {
		if (INVENTORY_ALLOCATED(i)) {
			...
			save_text_write_array(f, "queue.type", (int*)types, 2);
...
save_text_map_state(FILE *f)
{
	...
	for (uint ty = 0; ty < game.map.rows; ty += SAVE_MAP_TILE_SIZE) {
		for (uint tx = 0; tx < game.map.cols; tx += SAVE_MAP_TILE_SIZE) {
...
load_text_game_state(list_t *sections)
{
	...
	game.map.size = (game.map.col_size + game.map.row_size) - 9;
...
load_text_player_section(section_t *section)
{
	...
	game.player[n] = (player_t*)calloc(1, sizeof(player_t));
...
load_text_flag_section(section_t *section)
{
	/* Parse flag number. */
	int n = atoi(section->param);
	if (n >= (int)game.flag_limit) return -1;
...
load_text_building_section(section_t *section)
{
	...
	int n = atoi(section->param);
	if (n >= (int)game.building_limit) return -1;
...
load_text_building_state(list_t *sections)
{
	...
	for (uint i = 1; i < game.building_limit; i++) {
		if (BUILDING_ALLOCATED(i)) {
...
load_text_inventory_section(section_t *section)
{
	/* Parse building number. */
	int n = atoi(section->param);
	if (n >= (int)game.inventory_limit) return -1;
...
load_text_serf_section(section_t *section)
{
	/* Parse serf number. */
	int n = atoi(section->param);
	if (n >= (int)game.serf_limit) return -1;
	...
	list_foreach(&section->settings, elm) {
		...
		case SERF_STATE_KNIGHT_LEAVE_FOR_WALK_TO_FIGHT:
			if (!strcmp(s->key, "state.dist_col")) {
				...
				serf->s.leave_for_walk_to_fight.next_state = (serf_state_t)atoi(s->value);
			}
			break;
