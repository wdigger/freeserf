...
/* event_loop() has been turned into a SDL based loop.
 ...
 in game_loop_iter. */
void
event_loop_sdl_t::run(event_handler_t **handlers)
{
  ...
  while (SDL_WaitEvent(&event)) {
    ...
    switch (event.type) {
      case SDL_MOUSEBUTTONUP:
        ...
        {
          notify_click(event.button.x, event.button.y, (event_button_t)event.button.button);
          ...
          if (current_ticks - last_click[event.button.button-1] < MOUSE_TIME_SENSITIVITY &&
              ...
              event.button.y <= static_cast<int>(last_click_y + MOUSE_MOVE_SENSITIVITY))
          {
            notify_dbl_click(event.button.x, event.button.y, (event_button_t)event.button.button);
          }
